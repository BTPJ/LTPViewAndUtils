...
class FlowLayout(context: Context, attrs: AttributeSet?) : ViewGroup(context, attrs) {
    ...
    init {
        ...
        mChildHorizontalSpacing = typeArray.getDimensionPixelSize(R.styleable.FlowLayout_childHorizontalSpacing, 0)
        mChildVerticalSpacing = typeArray.getDimensionPixelSize(R.styleable.FlowLayout_childVerticalSpacing, 0)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (widthSpecMode == View.MeasureSpec.EXACTLY) {
            ...
            for (i in 0 until count) {
                ...
                val childWidthMeasureSpec = ViewGroup.getChildMeasureSpec(widthMeasureSpec, paddingLeft + paddingRight, childLayoutParams.width)
                val childHeightMeasureSpec = ViewGroup.getChildMeasureSpec(heightMeasureSpec, paddingTop + paddingBottom, childLayoutParams.height)
            ...
            resultHeight = when (heightSpecMode) {
                ...
                View.MeasureSpec.AT_MOST -> Math.min(childPositionY + maxLineHeight + paddingBottom, heightSpecSize)
                else -> heightSpecSize
            }
        } else {
            // 不计算换行，直接一行铺开
